...
class MultilabelRealdataTestCase(unittest.TestCase):
    def setUp(self):
        dataset_filepath = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "datasets/yeast_train.svm"
        )
    ...
    def test_mmc(self):
        ...
        assert_array_equal(
            qseq, np.array([117, 655, 234, 1419, 1350, 1224, 427, 890, 1447, 103])
        )
    ...
    def test_multilabel_with_auxiliary_learner_shlr(self):
        ...
        qseq = run_qs(trn_ds, qs, self.y, self.quota)
        assert_array_equal(
            qseq, np.array([1258, 805, 459, 550, 783, 964, 736, 1004, 38, 750])
        )
    ...
    def test_multilabel_with_auxiliary_learner_mmr(self):
        ...
        qseq = run_qs(trn_ds, qs, self.y, self.quota)
        assert_array_equal(
            qseq, np.array([1258, 1461, 231, 1198, 1498, 1374, 955, 1367, 265, 144])
        )
    ...
    def test_binary_minimization(self):
        ...
        qseq = run_qs(trn_ds, qs, self.y, self.quota)
        assert_array_equal(
            qseq, np.array([936, 924, 1211, 1286, 590, 429, 404, 962, 825, 30])
        )
    ...
    def test_cost_sensitive_random_pair_encoding(self):
        ...
        qseq = run_qs(trn_ds, qs, self.y, self.quota)
        assert_array_equal(
            qseq, np.array([149, 434, 1126, 719, 983, 564, 816, 732, 101, 1242])
        )
