...
class IrisTestCase(unittest.TestCase):
    ...
    def test_alce_lr(self):
        ...
        ds = Dataset(
            self.X + self.X_pool, self.y[:3] + [None for _ in range(len(self.X_pool))]
        )
        qs = ALCE(ds, cost_matrix, LinearRegression(), random_state=1126)
        qseq = run_qs(ds, qs, self.y_truth, self.quota)
        assert_array_equal(qseq, np.array([58, 118, 134, 43, 60, 139, 87, 78, 67, 146]))
    ...
    def test_alce_lr_embed5(self):
        ...
        qs = ALCE(ds, cost_matrix, LinearRegression(), embed_dim=5, random_state=1126)
        qseq = run_qs(ds, qs, self.y_truth, self.quota)
        assert_array_equal(qseq, np.array([106, 118, 141, 43, 63, 99, 65, 89, 26, 52]))
